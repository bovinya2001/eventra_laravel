...
<body class="bg-[#0a0a0f] text-[#f0f0f8] min-h-screen flex">
    ...
    <aside class="w-64 bg-[#12121a]/80 border-r border-white/5 flex flex-col min-h-screen fixed">
        <div class="p-6 border-b border-white/10">
            <div class="flex items-center gap-2">
                ...
                <div>
                    <div class="font-display font-bold text-sm">eventra</div>
                    <div class="text-xs text-violet-400">Admin workspace</div>
                </div>
            </div>
        </div>
        <nav class="flex-1 p-4 space-y-1">
            <a href="{{ route('admin.dashboard') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl bg-violet-600/20 text-violet-300 font-medium text-sm">
    ...
    <main class="ml-64 flex-1 p-8">
        <div class="mb-8">
            <div class="text-sm text-violet-400 uppercase tracking-widest mb-2 font-medium">Admin overview</div>
            <h1 class="font-display text-3xl font-black">Dashboard</h1>
            <p class="text-[#8888aa] text-sm mt-1">Welcome back, {{ auth()->user()->name }}!</p>
        </div>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            ...
            <div class="bg-[#12121a] border border-white/5 rounded-2xl p-6">
                <div class="flex items-center justify-between mb-4">
                    <div class="w-10 h-10 bg-violet-600/10 rounded-xl flex items-center justify-center">
        ...
        <div class="bg-[#12121a] border border-white/5 rounded-2xl p-6">
            <div class="flex justify-between items-center mb-6">
                <h2 class="font-display font-bold">Recent Events</h2>
                <a href="{{ route('admin.events.create') }}" class="bg-violet-600 hover:bg-violet-500 text-white text-sm px-4 py-2 rounded-xl transition-colors">+ New Event</a>
            </div>
            <div class="space-y-3">
